                                    ...
                                    if costo.apoyo_de_pasajes < 0:
                                        ...
                                        if costo.laborados > 31:
                                            ...
                                            if costo.apoyo_vist_familiar < 0:
                                                ...
                                                if costo.apoyo_salud < 0:
                                                    messages.error(request, '(Apoyo salud) La cantidad capturada debe ser mayor o igual 0')
                                                else:
                                                    if costo.estancia < 0:
                                                                            ...
                                                                            if costo.sdi_imss < 0:
                                                                                ...
                                                                                if costo.laborados_imss > 31:
                                                                                    ...
                                                                                    if form.is_valid():
                                                                                        ...
                                                                                        costo.complete = True
                                                                                        costo.save()
                                                                                        status = Status.objects.get(id=costo.status.id)
...
def CostoUpdate(request, pk):
    ...
    if user_filter.tipo.id in [4,9,10,11] and user_filter.distrito.id == costo.status.perfil.distrito.id: #Perfil RH
        tablas= DatosISR.objects.all()
        tabla_subsidio = TablaSubsidio.objects.all()
        tabla_vacaciones= TablaVacaciones.objects.all()
        tcesantias= TablaCesantia.objects.all()
...
def Costo_revisar(request, pk):
    ...
    if (usuario.tipo.id in (9,10,11)) or (usuario.tipo.id in (4,8,12) and usuario.distrito.id == costo.status.perfil.distrito.id): #Perfil RH
        ...
        costo.amortizacion_infonavit=Decimal(costo.amortizacion_infonavit)
        costo.fonacot=Decimal(costo.fonacot)
        costo.neto_catorcenal_sin_deducciones=Decimal(costo.neto_catorcenal_sin_deducciones)
        costo.complemento_salario_catorcenal=Decimal(costo.complemento_salario_catorcenal)
        costo.sueldo_diario=Decimal(costo.sueldo_diario)
        costo.sdi=Decimal(costo.sdi)
        costo.apoyo_de_pasajes=Decimal(costo.apoyo_de_pasajes)
        costo.imms_obrero_patronal=Decimal(costo.imms_obrero_patronal)
        costo.apoyo_vist_familiar=Decimal(costo.apoyo_vist_familiar)
        costo.estancia=Decimal(costo.estancia)
        costo.renta=Decimal(costo.renta)
        costo.apoyo_estudios=Decimal(costo.apoyo_estudios)
        costo.amv=Decimal(costo.amv)
        costo.gasolina=Decimal(costo.gasolina)
        costo.campamento=Decimal(costo.campamento)
        costo.total_deduccion=Decimal(costo.total_deduccion)
        costo.neto_pagar=Decimal(costo.neto_pagar)
        costo.sueldo_mensual_neto=Decimal(costo.sueldo_mensual_neto)
        costo.complemento_salario_mensual=Decimal(costo.complemento_salario_mensual)
        costo.sueldo_mensual=Decimal(costo.sueldo_mensual)
        costo.sueldo_mensual_sdi=Decimal(costo.sueldo_mensual_sdi)
        costo.total_percepciones_mensual=Decimal(costo.total_percepciones_mensual)
        costo.impuesto_estatal=Decimal(costo.impuesto_estatal)
        costo.sar=Decimal(costo.sar)
        costo.cesantia=Decimal(costo.cesantia)
        costo.infonavit=Decimal(costo.infonavit)
        costo.isr=Decimal(costo.isr)
        costo.lim_inferior=Decimal(costo.lim_inferior)
        costo.excedente=Decimal(costo.excedente)
        costo.tasa=Decimal(costo.tasa)
        costo.impuesto_marginal=Decimal(costo.impuesto_marginal)
        costo.cuota_fija=Decimal(costo.cuota_fija)
        costo.impuesto=Decimal(costo.impuesto)
        costo.subsidio=Decimal(costo.subsidio)
        costo.total_apoyosbonos_empleadocomp=Decimal(costo.total_apoyosbonos_empleadocomp)
        costo.total_apoyosbonos_agregcomis=Decimal(costo.total_apoyosbonos_agregcomis)
        costo.comision_complemeto_salario_bonos=Decimal(costo.comision_complemeto_salario_bonos)
        costo.total_costo_empresa=Decimal(costo.total_costo_empresa)
        costo.ingreso_mensual_neto_empleado=Decimal(costo.ingreso_mensual_neto_empleado)
        costo.apoyo_salud=Decimal(costo.apoyo_salud)
        #se agrego el bono
        costo.bono_total=Decimal(costo.bono_total)
        bonototal = Decimal(bonototal)
        if request.method =='POST' and 'Pdf' in request.POST:
...
def Empleado_Costo(request, pk):
    ...
    if user_filter.tipo.id in [4,9,10,11] or (user_filter.numero_de_trabajador == costo.status.perfil.numero_de_trabajador and user_filter.distrito == costo.status.perfil.distrito) or user_filter.tipo.id == 3:
        ...
        costo.total_apoyosbonos_empleadocomp=Decimal(costo.total_apoyosbonos_empleadocomp)
        if bonototal == None:
            costo.bonototal =Decimal(0)
        else:
            costo.bonototal=Decimal(bonototal)
        costo.total_apoyosbonos_agregcomis=Decimal(costo.total_apoyosbonos_agregcomis)
        costo.comision_complemeto_salario_bonos=Decimal(costo.comision_complemeto_salario_bonos)
        costo.total_costo_empresa=Decimal(costo.total_costo_empresa)
        costo.ingreso_mensual_neto_empleado=Decimal(costo.ingreso_mensual_neto_empleado)
...
def TablaCosto(request):
    ...
    if usuario.tipo.id in [4,8,9,10,11,12]: #Perfil RH
        ...
        context = {'costos':costos,'costo_filter':costo_filter,'salidas_list':salidas_list, 'baja': request.GET.get('baja', False), 'ids':ids,}
...
def VacacionesUpdate(request, pk):
    ...
    if user_filter.tipo.id in [4,9,10,11] and user_filter.distrito == descanso.status.perfil.distrito: #Perfil RH
        rh = Status.objects.get(perfil__id = user_filter.perfil.id, perfil__distrito_id = user_filter.distrito.id)
...
def VacacionesRevisar(request, pk):
    ...
    if user_filter.tipo.id in [9,10,11] or (user_filter.perfil.status.id == usuario.perfil.id and user_filter.distrito.id == usuario.perfil.distrito.id ) or (user_filter.tipo.id in [4] and user_filter.distrito.id == usuario.perfil.distrito.id):
        datos = Vacaciones.objects.filter(status_id=usuario.id).order_by(Cast('periodo', output_field=IntegerField()).desc()) #Identifico las vacaciones del usuario de la mas antigua a la mas actual
        actual = Vacaciones.objects.filter(status_id=usuario.id).order_by(Cast('periodo', output_field=IntegerField())).last()
        resultado = 0
        for dato in datos:
...
def Empleado_Datosbancarios(request, pk):
    ...
    if user_filter.tipo.id in [4,9,10,11] or (user_filter.perfil.id == datos.status.perfil.id and user_filter.distrito.id == datos.status.perfil.distrito.id) or user_filter.tipo.id == 3:
...
def obtener_distrito_y_division(distrito_name):
    if distrito_name == "MOTOS":
        return Distrito.objects.get(distrito="Planta Veracruz"), "MOTOS"
    elif distrito_name == "PETROLERA":
        return Distrito.objects.get(distrito="Planta Veracruz"), "PETROLERA"
    elif Distrito.objects.filter(distrito=distrito_name).exists():
        return Distrito.objects.get(distrito=distrito_name), ""
    return None, ""
                ...
                if status.fecha_ingreso != None:
                    if Costo.objects.filter(status__perfil__numero_de_trabajador = row[0], status__perfil__distrito__distrito = distrito):
                        ...
                        costo = Costo(status=status,neto_catorcenal_sin_deducciones=row[2],complemento_salario_catorcenal=row[3],sueldo_diario=row[4],
                                ...
                                campamento=row[11],apoyo_estudios=row[12],gasolina=row[13],amv=row[14],laborados=row[15],laborados_imss=row[16],sdi_imss=row[17],apoyo_salud=row[18],complete=True,)

                        costo.neto_catorcenal_sin_deducciones = Decimal(row[2])
                        costo.complemento_salario_catorcenal = Decimal(row[3])
                        costo.sueldo_diario = Decimal(row[4])
                        costo.amortizacion_infonavit = Decimal(row[5])
                        costo.fonacot = Decimal(row[6])
                        costo.apoyo_de_pasajes = Decimal(row[7])
                        costo.apoyo_vist_familiar = Decimal(row[8])
                        costo.estancia = Decimal(row[9])
                        costo.renta = Decimal(row[10])
                        costo.campamento= Decimal(row[11])
                        costo.apoyo_estudios= Decimal(row[12])
                        ...
                        costo.apoyo_salud = Decimal(row[18])
                        ahora = datetime.date.today()
...
def reporte_pdf_costo_incidencias(request,costo,bonototal):
    ...
    c.drawString(130, 150,str(cantidad_dias_vacacion))
    
    # Segunda columna
    c.drawString(425, 575, Decimal(salario_catorcenal_costo))
    c.drawString(425, 550, Decimal(salario_catorcenal))
    c.drawString(425, 525, Decimal(apoyo_pasajes))
    c.drawString(425, 500, Decimal(total_bonos))
    c.drawString(425, 475, Decimal(total_percepciones))
    c.drawString(425, 450, Decimal(prestamo_infonavit))
    c.drawString(425, 425, Decimal(prestamo_fonacot))
    c.drawString(425, 400, Decimal(total_deducciones))
    c.drawString(425, 375, Decimal(pagar_nomina))
                ...
                if 'btnAutorizar' in request.POST:
                    ...
                    if solicitud.autorizar_jefe == None:
                        ...
                        if vacacion.dias_disfrutados is None:
                            vacacion.dias_disfrutados = 0
                        if vacacion.dias_de_vacaciones is None:
                            vacacion.dias_de_vacaciones = 0
        ...
        if form.is_valid():
            if 'btnSolicitudAprobar' in request.POST:
                ...
                if solicitud.autorizar_jefe == None:
                    ...
                    solicitud = form.save(commit=False)
                    solicitud.comentario = solicitud.observacion_jefe
                    observaciones = solicitud.observacion_jefe
                    solicitud.autorizar = 1
                    solicitud.save()
                ...
                messages.success(request, 'Solicitud no autorizada')
...
def PdfFormatoEconomicos(request, pk):
    ...
    #gerente = Perfil.objects.get(pk = solicitud.perfil_gerente_id)
    ...
    #c.drawCentredString(170, 138, gerente.nombres+" "+gerente.apellidos)
    c.drawCentredString(170,125,'FIRMA GERENCIA')
    c.rect(70,123, 200, 12)
    c.rect(70,135, 200, 50)
    #c.drawCentredString(440, 138, jefe_inmediato.nombres+" "+gerente.apellidos)
...
@login_required(login_url='user-login')
@perfil_session_seleccionado
def tabla_registro_patronal(request):
    registros_patronales = RegistroPatronal.objects.all()

    context= {
        'registros_patronales':registros_patronales,

        }

    return render(request, 'proyecto/tabla_registro_patronal.html',context)

@login_required(login_url='user-login')
def tabla_registro_patronal_update(request, pk):
    user_filter = UserDatos.objects.get(user=request.user)
    item = RegistroPatronal.objects.get(id=pk)

    if request.method == 'POST':
        form = Registro_patronal_form(request.POST, instance=item)

        if form.is_valid():
            messages.success(request, 'Cambios guardados con éxito en el registro patronal')
            item = form.save(commit=False)
            nombre = Perfil.objects.get(numero_de_trabajador = user_filter.numero_de_trabajador, distrito = user_filter.distrito)
            item.editado = str("U:"+nombre.nombres+" "+nombre.apellidos)
            item.complete = True
            ...
            return redirect('tabla_registro_patronal')
    else:
        form = Registro_patronal_form(instance=item)

    context = {'form':form,'item':item}

    return render(request, 'proyecto/tabla_registro_patronal_update.html',context)
